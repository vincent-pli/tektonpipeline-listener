...
type Controller struct {
	...
	listenerTemplatesLister listers.ListenerTemplateLister
	listenerTemplatesSynced cache.InformerSynced
...
func NewController(
	...
	controller := &Controller{
		...
		listenerTemplatesLister: listenerTemplateInformer.Lister(),
		listenerTemplatesSynced: listenerTemplateInformer.Informer().HasSynced,
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	klog.Info("Starting listenerTemplate controller")

	// Wait for the caches to be synced before starting workers
	klog.Info("Waiting for informer caches to sync")
	if ok := cache.WaitForCacheSync(stopCh, c.deploymentsSynced, c.listenerTemplatesSynced); !ok {
...
func (c *Controller) enqueuelistenerTemplate(obj interface{}) {
	var key string
	var err error
